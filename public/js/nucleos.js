    ...
    <form method="post" class="form-usuario" id="formAgregar">
      <div class="grid-form">
        ...
        <div class="campo-grupo">
          <label>Nombre *</label>
          <input type="text" id="nombre" name="nucleo" required>
        </div>
        ...
        <div class="campo-grupo">
          ...
          <input type="file" name="img" class="input-preview" data-target="previewFondoAdd" accept="image/*">
  ...
  contenedor.innerHTML = `
    ...
    <form method="post" class="form-usuario" id="formEditar">
      ...
      <div class="grid-form">
       <div class="campo-grupo">
          <label>Nombre *</label>
          <input type="text" id="nombre" name="nucleo" required value="${datos.nucleo}">
        </div>
        <div class="campo-grupo container-textarea">
          <label>Descripción</label>
     ...
     <div class="contaniner-img-muestras">
            ...
            <div class="contenedor-preview ">
                    <img id="previewLogoAdd" src="${datos.logo}" alt="Previa Logo" >
            </div>

            <div class="contenedor-preview mt-2">
                      <img id="previewFondoAdd" src="${datos.img}" alt="Previa Fondo">
            </div>
...
if (agregarPrograma) {
  agregarPrograma.addEventListener("click", () => {
    ...
    inicializarFormulario("formAgregar", "nucleos/guardar", (res) => {
...
function renderizarTabla(datos, paginaActual = 1, limitePorPagina = 10) {
  ...
  datos.forEach((datos) => {
    ...
    fila.innerHTML = `
      ...
      <td>${escaparHTML(datos.nucleo)}</td>
      <td><img class="img-nucleo" src="${escaparHTML(datos.img)}"></td>
      <td><img class="img-nucleo" src="${escaparHTML(datos.logo)}"></td>
    `;
...
function abrirModalEdicion(programa) {
  ...
  inicializarFormulario("formEditar", "nucleos/actualizar", (res) => {
    AlertApp.show("Nucleo actualizado con éxito", "", "success");
...
function eliminarProgramasSeleccionados() {
  ...
  AlertApp.show(
    ...
    () => {
      ...
      fetch("nucleos/eliminacionMultiple", {
        method: "POST",
        headers: {
