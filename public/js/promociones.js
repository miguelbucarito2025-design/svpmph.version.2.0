  ...
  contenedor.innerHTML = `
    ...
    <input type="hidden" name="id" value="${datos.id}">
      <div class="grid-form-2col">
          <div class="campo-grupo">
             <label>Nombre de la Promoción *</label>
             <input type="text" name="promocion" value="${datos.promocion}"  required>
          </div>
          <div class="campo-grupo">
             <label>periodo *</label>
             <input type="text"  name="periodo" value="${datos.periodo}"  required>
          </div>
      </div>
  ...
  datos.forEach((rol) => {
    ...
    card.innerHTML = `
     ...
     ${svgEditar}
      ...
      <div class="card-img">
        <img src="${escaparHTML(imagen)}" alt="Flyer ${escaparHTML(rol.promocion || "Rol")}">
      </div>

      <div class="card-oferta-body">
        <h3 class="card-title">${escaparHTML(rol.promocion || "N/A")}</h3>
        
        <div class="card-details-rol">
                 <p class="card-subtitle"><strong>Periodo :</strong> ${escaparHTML(rol.periodo || "Descripción N/A")}</p>
        </div>
      </div>
    `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnAgregar) {
    btnAgregar.addEventListener("click", () => {
      ...
      inicializarFormulario("formAgregar", "promocion/guardar", (res) => {
        AlertApp.show("Promoción guardada con éxito", "", "success");
...
function abrirModalEdicion(rol) {
  ...
  inicializarFormulario("formEditar", "promocion/actualizar", (res) => {
    AlertApp.show("Promoción actualizada con éxito", "", "success");
    refrescarTabla(false);
  });
}
...
function eliminarOfertasSeleccionadas() {
  ...
  if (idsParaEliminar.length === 0) {
    AlertApp.show(
      "Atención",
      "Debe seleccionar al menos una Promocion para eliminar.",
      "warning",
    );
    return;
  }
  ...
  AlertApp.show(
    "Atención",
    "¿Seguro que deseas eliminar las Promociones seleccionadas? Esta acción no se puede deshacer.",
    "warning",
    () => {
      ...
      fetch("promocion/eliminacionMultiple", {
        method: "POST",
        headers: {
        ...
        .then((res) => {
          if (res.status === "success" || res.exito) {
            AlertApp.show(
              "Eliminado",
              "Las Promociones seleccionadas han sido removidas.",
              "success",
            );
            refrescarTabla(false);
          } else {
