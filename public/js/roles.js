...
function crearFormularioEditar(datos) {
  ...
  contenedor.innerHTML = `
    ...
    <input type="hidden" name="id" value="${datos.id_rol}">
      <div class="grid-form-2col">
          <div class="campo-grupo">
             <label>Nombre del Rol *</label>
             <input type="text" name="rol" value="${datos.rol}"  required>
          </div>
          <div class="campo-grupo">
             <label>Descripcción *</label>
...
function renderizarTabla(datos, paginaActual = 1, limitePorPagina = 3) {
  ...
  datos.forEach((rol) => {
    ...
    card.innerHTML = `
      ...
      <input type="checkbox" class="form-check-input check-oferta" data-id="${rol.id_rol}">
     <div class="container-button-card-header">
     <button type="button" class="btn-editar-card btn-terminos" title="Terminos">
     ${svgTerminos}
     </button>
     ...
     ${svgEditar}
      ...
      <div class="card-img">
        <img src="${escaparHTML(imagen)}" alt="Flyer ${escaparHTML(rol.rol || "Rol")}">
      </div>

      <div class="card-oferta-body">
        <h3 class="card-title">${escaparHTML(rol.rol || "Rol N/A")}</h3>
        
        <div class="card-details-rol">
                 <p class="card-subtitle"><strong>Descripción:</strong> ${escaparHTML(rol.descripcion || "Descripción N/A")}</p>
                 <p class="card-subtitle"><strong>Fecha de Modificación de los <br> Términos y Condiciones: </strong> ${escaparHTML(rol.fecha || " N/A")}</p>
        </div>
      </div>
    `;
    ...
    const btnTerminos = card.querySelector(".btn-terminos");
    btnTerminos.addEventListener("click", () => {
      abrirModalTerminos(rol);
    });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnAgregar) {
    btnAgregar.addEventListener("click", () => {
      ...
      inicializarFormulario("formAgregar", "roles/guardar", (res) => {
        AlertApp.show("Rol guardado con éxito", "", "success");
        refrescarTabla(true);
      });
    });
  }
...
function abrirModalTerminos(terminos) {
  const form = formularioTerminos(terminos);
  ...
  inicializarFormulario("formTerminos", "terminos/vincular", (res) => {
    AlertApp.show(
      "Terminos y Condiciones actualizado con éxito",
      "",
      "success",
    );
...
function abrirModalEdicion(rol) {
  ...
  inicializarFormulario("formEditar", "roles/actualizar", (res) => {
    AlertApp.show("Rol actualizado con éxito", "", "success");
    refrescarTabla(true);
  });
}
...
function eliminarOfertasSeleccionadas() {
  ...
  AlertApp.show(
    ...
    () => {
      ...
      fetch("roles/eliminacionMultiple", {
        method: "POST",
        headers: {
